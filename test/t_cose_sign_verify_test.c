...
static int_fast32_t known_good_test(int cose_algorithm_id, struct q_useful_buf_c signed_message)
{
    ...
    result = make_ecdsa_key_pair(cose_algorithm_id, &key_pair);
    if(result) {
        return_value = 3000 + (int32_t)result;
        goto Done2;
    }
...
int_fast32_t sign_verify_known_good_test(void)
{
    ...
    if (return_value) {
        return_value = 10000 + return_value;
        goto Done;
    }

#ifndef T_COSE_DISABLE_ES384
    return_value = known_good_test(T_COSE_ALGORITHM_ES384,
                                   Q_USEFUL_BUF_FROM_BYTE_ARRAY_LITERAL(signed_cose_made_by_psa_crypto_es384));
    if (return_value) {
        return_value = 20000 + return_value;
        goto Done;
    }
#endif /* T_COSE_DISABLE_ES384 */

#ifndef T_COSE_DISABLE_ES512
    return_value = known_good_test(T_COSE_ALGORITHM_ES512,
                                   Q_USEFUL_BUF_FROM_BYTE_ARRAY_LITERAL(signed_cose_made_by_ossl_crypto_es512));
    if (return_value) {
        return_value = 30000 + return_value;
        goto Done;
    }
#endif /* T_COSE_DISABLE_ES512 */
